...
- Columnas recomendadas (nombres exactos o cercanos):
  MotorPower_kW / MotorPowerEfective_kW / MotorPowerInstalled_kW
  Poles
  r_trans (o RelacionTransmision)
  MotorSpeedMin_rpm / MotorSpeedMax_rpm
  Pump_N_min_rpm / Pump_N_max_rpm (opcional)
  motor_J_kgm2, j_driver_total_kgm2, j_driven_total_kgm2, impeller_J_kgm2, impeller_D_mm
  H0_m, K_m_s2, eta_a, eta_b, eta_c, rho_kgm3, n_ref_rpm, Q_min_m3h, Q_max_m3h
"""
...
def pick(cols, *candidates, default=None):
    """elige la primera columna disponible"""
    for c in candidates:
...
# Primera columna es TAG (única)
first_col = df.columns[0]
col_tag = first_col

cols = set(df.columns)

# mapeos muy cortos (mantengo flexibilidad mínima)
...
tags = df[col_tag].astype(str).tolist()  # ya sabemos que son únicos
...
Aceleración por par:
\[
\dot n_{\text{torque}} = \frac{60}{2\pi}\frac{T_{\text{nom}}}{J_{\text{eq}}}
\]
Tiempos:
\[
...
with cC:
    # si no hay par nominal, lo estimamos con P y n_max
...
Δn = max(0.0, n_fin - n_ini)
accel_torque = (60.0/(2.0*math.pi)) * (T_nom / max(J_eq, 1e-9))
t_par   = Δn / max(accel_torque, 1e-9)
t_rampa = Δn / max(ramp_rpm_s, 1e-9)
t_nohyd = max(t_par, t_rampa)

st.success(f"Δn={Δn:.1f} rpm | ẋ_n={accel_torque:.1f} rpm/s  →  t_par={t_par:.2f} s, t_rampa={t_rampa:.2f} s, **t_final(sin)={t_nohyd:.2f} s**")
...
def eta_of_Q(Q): return eta_a + eta_b*Q + eta_c*Q**2
def H_of_Q(Q):   return H0 + K*Q**2
...
alpha_default = (Qref_h / max(n_ref, 1.0))  # m3/h por rpm de bomba
...
def simulate_with_hyd(n0, n1, dt, T_nom, J_eq, r_tr, alpha_m3s_per_rpm,
    ...
    while n < n1:
        ...
        accel_torque = (60/(2*math.pi)) * ((T_nom - T_mot_load)/max(J_eq,1e-9))  # rpm/s
...
if not any(pd.isna([J_eq, r_tr, H0, K, rho])):
    sim = simulate_with_hyd(
        ...
        alpha_m3s_per_rpm=alpha, H0=H0, K=K, eta_a=eta_a, eta_b=eta_b, eta_c=eta_c,
        rho=rho, ramp_rpm_s=ramp_rpm_s
    )
...
summary = pd.DataFrame([{
    ...
    "n_ini_rpm": n_ini, "n_fin_rpm": n_fin, "delta_n_rpm": Δn,
