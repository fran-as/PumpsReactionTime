...
with colL:
    ...
    if np.isnan(J_bushing_driver):
        J_bushing_driver = 0.10 * J_driver  # fallback 10%
...
if not feasible:
    ...
    i0 = int(idx_neg[0])
    n_cross = n_m_grid[i0]
    fig_int.add_vline(x=float(n_cross), line_dash="dash", line_color="#b00020")
    fig_int.add_annotation(x=float(n_cross), y=float(np.nanmax(dt_dn[np.isfinite(dt_dn)])),
                           text="Par insuficiente", showarrow=False, yshift=10)
...
# Mostrar tiempo hidráulico y criterio/avisos (uno debajo del otro)
...
if feasible:
    ...
    pill_error(
        f"Par motor insuficiente: el par resistente supera al disponible alrededor de "
        f"nₚ≈{fmt_num(n_p_cross,'rpm',0)}, nₘ≈{fmt_num(n_m_cross,'rpm',0)}, Q≈{fmt_num(Q_cross,'m³/h',0)}. "
        "Aumente el torque disponible, reduzca el caudal o ajuste la transmisión."
    )
...
if feasible and len(omega_m_grid) > 1:
    ...
    dt_series = np.append(dt_series, dt_series[-1])
else:
    dt_series = np.full_like(n_m_grid, np.nan, dtype=float)
